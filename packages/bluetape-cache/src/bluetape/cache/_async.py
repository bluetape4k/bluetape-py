...
class AsyncTTLCache(Generic[K, V]):  # noqa: UP046 - public generics intentionally use TypeVar
    ...
    async def _release_waiter_cancellation_safe(
        ...
        release_coroutine = self._release_waiter(flight, task, lock)
        try:
            ...
            await self._release_waiter_without_task(
                flight,
                task,
                lock,
    ...
    async def _release_waiter_without_task(
        self,
        flight: _AsyncFlight[K, V],
        task: asyncio.Task[V],
        lock: asyncio.Lock,
        *,
        ...
        while True:
            try:
                ...
                cancelled = True
        try:
            task_to_cancel = self._release_waiter_locked(flight, task)
        finally:
            lock.release()
    ...
    def _release_waiter_locked(
        self,
        flight: _AsyncFlight[K, V],
        task: asyncio.Task[V],
    ) -> asyncio.Task[V] | None:
        flight.waiters -= 1
        if flight.waiters != 0 or task.done():
            return None
