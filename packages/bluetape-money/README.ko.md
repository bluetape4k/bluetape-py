...
표준 라이브러리만 사용하는 ISO 4217 통화와 정확한 `Decimal` money 값을
호출자가 제공하는 exchange rate와 함께 제공합니다.
...
문자열만 받습니다. 모든 이진 부동소수와 boolean은 산술 전에 거부합니다. 연산은
패키지 로컬 Decimal context를 사용하며 호출자의 ambient precision을 변경하거나
상속하지 않습니다.

## ISO 4217 스냅샷

통화 메타데이터는 commit한 SIX ISO 4217 current List One snapshot에서 오프라인으로
생성합니다. 정확한 XML과 provenance manifest는
`docs/research/sources/iso4217/`에 있으며 `scripts/update-iso4217.py`가 명시적 local
입력으로 Python table을 결정적으로 재생성합니다. Import, build, 일반 사용은
network에 접근하지 않습니다. `XXX`와 `XTS`는 제외합니다.

## 반올림과 환율

산술은 자동으로 반올림하지 않습니다. `quantize()`와 `minor_units()`가 caller가
선택한 stdlib Decimal rounding을 포함한 minor-unit 정책을 명시합니다. 현재 ISO
row에 minor unit이 없는 currency는 이 연산을 거부합니다.

`ExchangeRate`는 caller가 제공하는 양의 exact base-to-quote 값입니다. `convert()`는
quantize하지 않고 양방향을 지원합니다. 패키지는 FX provider, freshness policy,
